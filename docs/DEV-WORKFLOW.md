...
На Linux/macOS есть `sonyd --simulated` — встроенный симулятор устройства.
На Windows он недоступен (нет IPC), а у GUI своего `--simulated` нет.
Первая задача в roadmap для удобства разработки — добавить в GUI флаг
`--simulated`, который подставляет `FakeTransport` вместо Bluetooth. Тогда
UI/темы/трей можно крутить без наушников и без разряда батареи.
