...
ViewPage {
    ...
    // The chart takes whatever height is left, never less than it needs to
    // read; a window shorter than that scrolls instead of squashing it.
    WheelScroll { flickable: batteryFlick }
    Flickable {
        id: batteryFlick
        anchors.fill: parent
        contentHeight: Math.max(height, batteryColumn.implicitHeight + 22 + appWindow.pageMargin)
        boundsBehavior: Flickable.StopAtBounds
        interactive: false
        clip: true
        ScrollBar.vertical: ScrollBar { policy: batteryFlick.contentHeight > batteryFlick.height ? ScrollBar.AsNeeded : ScrollBar.AlwaysOff }

        ColumnLayout {
            id: batteryColumn
            x: appWindow.pageMargin
            y: 22
            width: batteryFlick.width - 2 * appWindow.pageMargin
            height: batteryFlick.contentHeight - 22 - appWindow.pageMargin
            spacing: 16
                ...
                Repeater {
                    model: {
                        ...
                        var rate = controller.batteryDischargeRate > 0
                                 ? appWindow.tr("battery_rate_value").arg(controller.batteryDischargeRate.toFixed(1))
                                 : controller.connected && !controller.isCharging ? appWindow.tr("battery_estimate_pending") : dash
